...
export async function saveMarkdownOverride(url, rawBody, meta) {
  ...
  if (!token) {
    return {
      ok: false,
      error: "Not logged in — this only saved for your current tab. Sign in with GitHub to make it real.",
    };
  }
  try {
    const res = await fetch(`${APIARY_API}/content`, {
      method: "POST",
...
export function hasSessionEdit(url) {
  return Object.prototype.hasOwnProperty.call(SESSION_OVERRIDES, url);
}
...
export async function fetchMarkdown(url) {
  ...
  if (Object.prototype.hasOwnProperty.call(SESSION_OVERRIDES, url)) {
    ...
    const repoPath = url.replace(/^\.\//, "");
    const apiUrl =
      APIARY_API && !APIARY_API.includes("REPLACE-WITH")
        ? `${APIARY_API}/content?path=${encodeURIComponent(repoPath)}`
        : null;
    if (apiUrl) {
