...
import {
  Star, DollarSign, X as XIcon, Check, Plus, Minus, ChevronLeft, ChevronRight, ChevronDown,
  Calendar as CalendarIcon, HelpCircle, Pencil, Trash2, Car, Route, Smartphone, Bed, Plane,
  Info, CheckCircle2, AlertTriangle, XCircle, Lightbulb, Loader2, Menu as MenuIcon,
  MoreHorizontal, MessageCircle, Phone, ExternalLink,
} from "lucide-react";
import { fetchMarkdown, fetchJSON } from "../lib/content.js";
...
import { FoundationTag } from "../components/PreviewEngine.jsx";
import { PageOutline } from "./GetStartedPage.jsx";

const ICON_LOOKUP = {
  star: Star, "dollar-sign": DollarSign, times: XIcon,
  check: Check, plus: Plus, minus: Minus,
  "angle-left": ChevronLeft, "angle-right": ChevronRight,
  "angle-down": ChevronDown, "calendar-alt": CalendarIcon,
  "question-circle": HelpCircle, pencil: Pencil, trash: Trash2,
  "car-crash": Car, route: Route, "mobile-alt": Smartphone,
  bed: Bed, plane: Plane, "info-circle": Info,
  "check-circle": CheckCircle2, "exclamation-triangle": AlertTriangle,
  "exclamation-circle": XCircle, lightbulb: Lightbulb,
  "spinner-third": Loader2, bars: MenuIcon, "ellipsis-h": MoreHorizontal,
  "web-chat-speech-bubbles": MessageCircle, "mobile-phone": Phone,
    ...
    <div style={{ border: "1px solid var(--color-line)", borderRadius: 8, overflow: "hidden" }}>
      ...
      {data.SPACING_SCALE.map((s, i) => (
        <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr", padding: "10px 14px", borderTop: "1px solid var(--color-line)", fontSize: 15 }}>
          <span style={{ fontWeight: 600 }}>{s.px}</span>
          <span style={{ fontFamily: "monospace", color: "var(--color-faint)" }}>{s.token}</span>
          <span style={{ color: "var(--color-body)" }}>{s.desc}</span>
        </div>
      ))}
    </div>
  );
        ...
        <div key={w.weight} style={{ marginBottom: 24 }}>
          ...
          {w.sizes.map((sz, i) => (
            <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 13, color: "var(--color-faint)", fontFamily: "monospace" }}>
                {sz.s}px / {sz.l}px line-height
              </span>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
      {data.ELEVATION.map((e, i) => (
        <div key={i} style={{ width: 100, height: 80, borderRadius: 8, background: "#fff", boxShadow: e.shadow, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "var(--color-faint)" }}>
          {e.name}
        </div>
      ))}
    </div>
...
function TokensPage() {
  ...
  useEffect(() => {
    fetchJSON("./content/tokens.json").then(setTokens);
  }, []);
  if (!tokens)
    return (
    ...
    <div>
      {tokens._meta?.note && (
        <div style={{ borderLeft: "3px solid var(--color-success-border)", background: "var(--color-success-bg)", padding: "12px 16px", fontSize: 14, color: "var(--color-body)", lineHeight: 1.6, marginBottom: 24, borderRadius: "0 8px 8px 0" }}>
          <strong style={{ color: "var(--color-success-text)" }}>Real RAA data — </strong>
          {tokens._meta.note}
        </div>
      )}
...
export function FoundationsPage({ foundationKey, foundationLabel }) {
  const [doc, setDoc] = useState(null);
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);
  const [displayLabel, setDisplayLabel] = useState(foundationLabel);
  useEffect(() => {
    setDisplayLabel(foundationLabel);
  }, [foundationLabel]);
  useEffect(() => {
    let cancelled = false;
    setDoc(null);
    setData(null);
    setError(null);
    Promise.all([fetchMarkdown(`./content/foundations/${foundationKey}/v1.md`), fetchJSON(`./content/foundations/${foundationKey}/data.json`).catch(() => ({}))])
      .then(([d, dt]) => {
        if (!cancelled) {
          setDoc(d);
          setData(dt);
  ...
  return (
    <div>
      <GrayBand
        ...
        extra={<FoundationTag />}
        description={
          foundationKey === "design-tokens"
            ? "The real source of truth — edit content/tokens.json and every value here (and eventually every component spec) updates."
            : "Colour, spacing, elevation, and typography carry real captured or documented values."
        }
        onTitleSave={setDisplayLabel}
      />
