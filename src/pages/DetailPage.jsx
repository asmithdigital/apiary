/* =============================================================================
   PAGE: DetailPage — rebuilt from real extracted values, not patched onto
...
import { VersionPicker, LoadingRow } from "../components/MarkdownView.jsx";
import { ComponentPreview } from "../components/PreviewEngine.jsx";
...
function useComponentContent(id, version) {
  ...
  useEffect(() => {
    ...
    setDoc(null);
    setError(null);
    Promise.all([fetchJSON(`./content/components/${id}/specs.json`), fetchMarkdown(`./content/components/${id}/${version}.md`)])
      .then(([s, d]) => {
        if (!cancelled) {
          setSpecs(s);
          setDoc(d);
...
function ComponentTabsBody({ id, version }) {
  const versions = sortVersions(store.MANIFEST.components[id] || ["v1"]);
                  ...
                  <div className="apy-spec-token-list">
                    {specs.tokenFindings.map((f, i) => (
                      <div key={i} className="apy-spec-token-row">
                        ...
                        <span className="apy-spec-token-code">
                          <ColorSwatch value={f.value} />
                          {f.token || f.value}
                        </span>
                      </div>
                    ))}
...
export function DetailPage({ id, name }) {
  const versionsSorted = sortVersions(store.MANIFEST.components[id] || ["v1"]);
...
export function FamilyPage({ family }) {
  ...
  useEffect(() => {
    fetchJSON("./content/nav.json").then((nav) => {
      const map = {};
      nav.components.forEach((c) => {
        map[c.id] = c.name;
      });
      setNames(map);
    });
  }, []);
  const versionsSorted = sortVersions(store.MANIFEST.components[selectedId] || ["v1"]);
  const [version, setVersion] = useState(versionsSorted[versionsSorted.length - 1]);
  useEffect(() => {
    const v = sortVersions(store.MANIFEST.components[selectedId] || ["v1"]);
