/* =============================================================================
   COMPONENT: Editable — Stage 1 of the editing interface. Hover any real
   content, click the pencil, edit as plain markdown with a small toolbar,
   Save updates it for this session only (lib/content.js's SESSION_OVERRIDES
...
import { saveMarkdownOverride, fetchMarkdown } from "../lib/content.js";
import { APIARY_API } from "../lib/config.js";
import { MarkdownBody } from "./MarkdownView.jsx";
...
export function AuthButton() {
  ...
  useEffect(() => {
    ...
    if (match && hash.includes("auth-success")) {
      ...
      fetch("https://api.github.com/user", { headers: { Authorization: `Bearer ${token}` } })
        .then((r) => r.json())
        .then((u) => {
          sessionStorage.setItem("apiary_gh_user", u.login);
          setUser(u.login);
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
        <span style={{ color: "var(--color-faint)" }}>
          Signed in as <strong style={{ color: "var(--color-ink)" }}>{user}</strong>
        </span>
        <button
          onClick={() => {
            sessionStorage.removeItem("apiary_gh_token");
            sessionStorage.removeItem("apiary_gh_user");
            setUser(null);
          }}
          style={{ background: "none", border: "1px solid var(--color-line)", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}
        >
          Sign out
        </button>
      </div>
  ...
  return (
    <a href={`${APIARY_API}/auth/start`} className="apy-btn-primary" style={{ padding: "6px 14px", fontSize: 13, textDecoration: "none" }}>
      Sign in with GitHub
    </a>
  );
}
...
export function EditableMarkdown({ url, doc, onSaved }) {
  ...
  const save = async () => {
    setSaving(true);
    setSaveMsg(null);
    const result = await saveMarkdownOverride(url, draft, doc.meta);
  ...
  if (!editing) {
    return (
      <div className="apy-editable-wrap">
        <button className="apy-edit-pencil" onClick={startEdit} title="Edit this content">
          ✎ Edit
        </button>
  ...
  return (
    <div className="apy-edit-panel">
      ...
      <textarea ref={textareaRef} value={draft} onChange={(e) => setDraft(e.target.value)} className="apy-edit-textarea" rows={16} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderTop: "1px solid var(--color-line)" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setEditing(false)} style={{ padding: "6px 14px", fontSize: 13, border: "1px solid var(--color-line)", background: "#fff", borderRadius: 6, cursor: "pointer" }}>
            Cancel
          </button>
          <button onClick={save} disabled={saving} className="apy-btn-primary" style={{ padding: "6px 16px", fontSize: 13 }}>
            {saving ? "Saving…" : "Save"}
          </button>
        </div>
      </div>
...
export function EditableTitle({ title, onSave }) {
  ...
  if (!editing) {
    return (
      <span className="apy-editable-title-wrap">
        <h1>{title}</h1>
        <button className="apy-edit-pencil-inline" onClick={() => { setDraft(title); setEditing(true); }} title="Edit title">
          ✎
        </button>
      </span>
    );
  ...
  return (
    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <button onClick={() => { onSave(draft); setEditing(false); }} className="apy-btn-primary" style={{ padding: "6px 14px", fontSize: 13 }}>
        Save
      </button>
      <button onClick={() => setEditing(false)} style={{ padding: "6px 14px", fontSize: 13, border: "1px solid var(--color-line)", background: "#fff", borderRadius: 6, cursor: "pointer" }}>
        Cancel
